#!/usr/bin/env python3
"""
Problema Exotérmico 2D - Bloco 1m x 1m (Temperatura Imposta)
Equações acopladas: temperatura (Tp) e geração de calor (teq)
Condições de contorno: Tp=25°C em todas as bordas
...
# Condição de contorno Dirichlet: Tp=25°C em todas as bordas
def boundary(x, on_boundary):
    return on_boundary
Tp_bc = Constant(25.0)
bc = DirichletBC(W.sub(0), Tp_bc, boundary)
...
# Resultados
os.makedirs('resultados_dirichlet', exist_ok=True)
times = []
Tp_center = []
teq_center = []
Q_values = []
...
for n in range(n1):
    ...
    if step % 1 == 0:
        ...
        Tp_file = File(f"resultados_dirichlet/Tp_{step:03d}.pvd")
        teq_file = File(f"resultados_dirichlet/teq_{step:03d}.pvd")
        Tp_file << u.sub(0)
        teq_file << u.sub(1)
...
plt.tight_layout()
plt.savefig('resultados_dirichlet/resultados_exotermico_dirichlet.png', dpi=300, bbox_inches='tight')
plt.show()

# Salvar dados numéricos
np.savetxt('resultados_dirichlet/dados_centro.txt', 
           np.column_stack([times, Tp_center, teq_center, Q_values]),
           header='Tempo(h) Temperatura(C) TempoEquivalente(s) Q(W/m3)',
           fmt='%.6e')

print("\n✅ Simulação exotérmica (Temperatura Imposta) concluída com sucesso!")
print("📊 Gráficos salvos em 'resultados_dirichlet/resultados_exotermico_dirichlet.png'")
print("📁 Arquivos de saída salvos na pasta 'resultados_dirichlet/'") 