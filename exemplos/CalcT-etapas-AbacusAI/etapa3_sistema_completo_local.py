#!/usr/bin/python3
"""
ETAPA 3 - Sistema Completo (Versão Local)
...
def main():
    print("=== ETAPA 3 - SISTEMA COMPLETO (LOCAL) ===")
    ...
    # Caminhos dos arquivos (adaptados para local) - usando malha refinada
    base_path = "Uploads"
    ...
    # Definir espaços de funções usando functionspace em vez de FunctionSpace
    ...
    # Abrir arquivos XDMF para salvar evolução temporal
    try:
        xdmf_temp = io.XDMFFile(MPI.COMM_WORLD, "etapa3_temperatura.xdmf", "w")
        xdmf_teq = io.XDMFFile(MPI.COMM_WORLD, "etapa3_teq.xdmf", "w")
        xdmf_Q = io.XDMFFile(MPI.COMM_WORLD, "etapa3_Q.xdmf", "w")
    ...
    try:
        ...
        print(f"  - etapa3_temperatura.xdmf ({total_steps} passos de tempo)")
        print(f"  - etapa3_teq.xdmf ({total_steps} passos de tempo)")
        print(f"  - etapa3_Q.xdmf ({total_steps} passos de tempo)")
    except Exception as e:
