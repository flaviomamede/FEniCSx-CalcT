...
#!/usr/bin/python3
"""
ETAPA 2 - Condução com Fonte Constante
Baseado na ETAPA 1, adiciona termo de fonte constante Q_constante = 10000.0
"""
...
def main():
    print("=== ETAPA 2 - CONDUÇÃO COM FONTE CONSTANTE ===")
    ...
    dt = 3600.0       # s (1 hora)
    Q_constante = 10000.0  # W/m³ (fonte de calor constante)
    ...
    print(f"  Q_constante = {Q_constante} W/m³")
    print(f"  dt = {dt} s")
    ...
    # Forma bilinear - aplicar em todo o domínio
    ...
    # Termo fonte em todo o domínio (temperatura anterior + fonte constante)
    L = (rho * ce / dt) * T_anterior * v * ufl.dx
    
    # Adicionar fonte constante apenas nos domínios ativos
    for domain_id in dominios_ativos:
        print(f"  Adicionando fonte constante no domínio {domain_id}")
        L += Q_constante * v * dx(domain_id)
    ...
    # Executar 3 passos de tempo para ver evolução
    for step in range(1, 4):
        ...
        try:
            ...
            # Verificar se temperatura aumentou (esperado com fonte de calor)
            if step > 1 and T_mean > T_inicial:
                print(f"  ✓ Temperatura aumentou devido à fonte de calor")
    ...
    # Salvar resultados
    output_file = "/home/ubuntu/etapa2_resultado.xdmf"
